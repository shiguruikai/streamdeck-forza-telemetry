...
type: Guide
title: "Settings"
description: "The Stream Deck SDK provides support for managing settings associated with your plugin. This is useful when you want to provide a higher level of configurabi..."
resource: https://docs.elgato.com/streamdeck/sdk/guides/settings/
tags: [sdk, settings, storage]
timestamp: 2026-07-11T20:01:18.314160+09:00
---
...
## Overview
Global and action settings share similarities in their APIs, and both can be managed from either the application-layer, or [property inspector (UI)](property-inspector.md). Additionally, once settings have been set, the adjacent environment is notified of the update, for example:
...
The following table provides an overview of the common functions and events:
  ...
  * [Writing](settings.md#writing-action-settings): 
    * `ev.action.setSettings(settings)`1
  * [Reading](settings.md#reading-action-settings): 
    * `ev.payload.settings`
    * `ev.action.getSettings()`1
  * [Changed](settings.md#action-settings-changed): 
...
In this context, `ev` are event arguments associated with an event emitted within the `SingletonAction` or from `streamDeck.*` for events that are associated with an action.
  * [Writing](settings.md#writing-global-settings): 
    * `streamDeck.settings.setGlobalSettings(settings)`
  * [Reading](settings.md#reading-global-settings): 
    * `streamDeck.settings.getGlobalSettings()`
  * [Changed](settings.md#global-settings-changed): 
...
Security
Security-sensitive settings, such as API keys, should always be persisted using [global settings](settings.md#global-settings), never action settings. Action settings are stored as plain-text and are included when exporting Stream Deck profiles, and in their nature action settings are not secure.
### Reading Settings
...
Type safety
Defining the type of your settings provides insight into what their type might be, but does not guarantee their underlying type. [Learn more about type-safety to prevent runtime errors](settings.md#type-safety).
