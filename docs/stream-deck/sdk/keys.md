...
type: Guide
title: "Keys"
description: "One of the two primary types of actions available to Stream Deck plugins; keys are found on all Stream Deck devices, and allow users to activate your plugin'..."
resource: https://docs.elgato.com/streamdeck/sdk/guides/keys/
tags: [sdk, keys]
timestamp: 2026-07-11T20:01:18.309361+09:00
---
...
## What Are Keys
Keys are your plugin's actions located on a Stream Deck canvas, for example on Stream Deck XL, Stream Deck Pedal, etc. They provide visual information to users in the form of a [title](keys.md#titles) and [image](keys.md#images), and can be activated by a user interacting with a physical Stream Deck device.
![Screenshot of Stream Deck software highlighting an action key](https://docs.elgato.com/img/streamdeck/sdk/keys.png)
## States
The state of a key action, configured within the [manifest](manifest.md#action-states), determines an action's behavior and defaults, for example the image shown on the canvas. All key actions must have at least one state, however they can have multiple states.
### Multi-State Keys
...
Automatic toggling
If you would prefer to control states yourself, you can also [opt out of automatic toggling](manifest.md#action-disableautomaticstates) within the manifest.
...
When your action has multiple states, it's important to assign a [`Name`](manifest.md#state-name) to each state, within the manifest. Doing so allows users to specify their desired state when using your action in a multi-action.
...
## Titles
All actions have a title; in the case of a key action, the title is rendered either at the top, middle, or bottom of the key, on-top of the image. Your plugin can define a [default title](manifest.md#state-title) within the manifest, and update it using the [`setTitle`](keys.md#settitle) command[†](keys.md#display-precedence).
### Setting Titles
...
## Images
Your plugin can update an action key's image[†](keys.md#display-precedence) using the [`setImage`](keys.md#setimage) command. The `setImage` function accepts a path to an image file or an [image data URL](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs) with base64 encoded data. Stream Deck supports multiple image formats:
...
## Temporary Feedback
There may be times when you want to show user temporary feedback on a key, for example when an action succeeds or fails. To achieve this, you can use the [`showOk`](keys.md#showok) and [`showAlert`](keys.md#showalert) functions on the `action`.
